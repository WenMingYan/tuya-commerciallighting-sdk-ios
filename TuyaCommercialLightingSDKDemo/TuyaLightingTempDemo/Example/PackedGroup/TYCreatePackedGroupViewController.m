...
@interface TYCreatePackedGroupViewController ()<UITableViewDelegate, UITableViewDataSource>
...
@implementation TYCreatePackedGroupViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    
    long long projectId = TYCacheManager.sharedInstance.projectId;
    TuyaSmartPackedGroup *group = [[TuyaSmartPackedGroup alloc] initWithGroupId:@"" projectId:projectId];
    ...
    [group getAvailableDevices2JoinPackedGroupWithAreaId:self.areaId
        ...
        NSArray<TuyaSmartDeviceModel *> *targetArray = [devices filteredArrayUsingPredicate:pre];
...
- (void)rightButtonAction:(id)sender {
    ...
    UIAlertAction *confirmAction =[UIAlertAction actionWithTitle:NSLocalizedString(@"Done", nil)
        ...
        [SVProgressHUD show];
        long long projectId = TYCacheManager.sharedInstance.projectId;
        [TuyaSmartPackedGroupManager createPackedGroupWithProjectId:projectId
                                                             areaId:self.areaId
